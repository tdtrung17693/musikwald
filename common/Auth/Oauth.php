...
class Oauth {
    ...
    {
        if (Auth::user()) {
           return View::make('shared::oauth/popup')->with('status', 'ALREADY_LOGGED_IN');
        }
    ...
    {
        return response()->view('shared::oauth.popup', ['status' => 'ERROR', 'data' => $message], 500);
    }
    ...
    {
        $view = View::make('shared::oauth/popup')->with('status', $status);
