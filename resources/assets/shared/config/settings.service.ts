...
export class Settings {
     ...
     */
    public setMultiple(settings: Config, fireEvent = false) {
     ...
     * Set single setting.
     */
    public set(name: keyof Config|any, value: Config[keyof Config]|any, fireEvent = false) {
     ...
     */
    public get(name: keyof Config|any, defaultValue: any = null): any {
     ...
     */
    public has(name: keyof Config): boolean {
     ...
     */
    public getJson(name: keyof Config, defaultValue: any = null) {
    ...
    public getBaseUrl(forceServerUrl = false): string {
        ...
        if (this.has('base_url') && (this.get('app.environment') === 'production' || forceServerUrl)) {
    ...
    public getAssetUrl(suffix?: string): string {
        let uri = (this.get('app.assetsUrl') || this.getBaseUrl());
        const prefix = this.get('app.assetsPrefix');

        // in production assets will be in "client" sub-folder
        if (this.get('app.environment') === 'production' && prefix) {
            uri += prefix + '/';
        }
