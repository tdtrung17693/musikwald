...
import {FilterablePage} from 'app/web-player/filterable-page/filterable-page';
import {Player} from 'app/web-player/player/player.service';

@Injectable()
export class LibraryUploadedTracks extends FilterablePage<Track> {
    /**
    ...
    public remove(tracks: Track[]|Track) {
        ...
        const request = this.http.delete('tracks', {ids});
    ...
    public fetch(params: { page?: number; query?: string } = {page: 1}): Observable<PaginationResponse<Track>> {
        ...
        return this.http.get('user/library/uploaded-tracks', params);
    }
