...
export class PlaylistComponent implements OnInit, OnDestroy {
    ...
    public reorderPlaylist(e: CdkDragDrop<any>) {
        const tracks = this.playlist.getTracks();
        moveItemInArray(tracks, e.previousIndex, e.currentIndex);
        const ids = tracks.map(track => track.id);
        this.playlists.changeTrackOrder(this.playlist.get().id, ids).subscribe();
    }
}
