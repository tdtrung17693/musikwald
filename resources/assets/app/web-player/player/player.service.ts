        ...
        this.state.onChange$.subscribe(type => {
            if (type === 'PLAYBACK_STARTED') {
                ...
                if (cuedTrack) {
                    ...
                    this.title.setTitle(`${cuedTrack.name} - ${cuedTrack.artists[0]}`);
                }
                ...
                const titleNode = this.meta.getTag('property="og:title"');
                this.title.setTitle(titleNode ? titleNode.content : this.settings.get('branding.site_name'));
            }
        });
    }
